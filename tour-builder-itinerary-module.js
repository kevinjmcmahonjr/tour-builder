...
let guiElements ={
    calendar: $(".calendar-field")[0],
    startDate: $('.starting-date-display'),
    endDate: $('.ending-date-display'),
    numberOfDays: $(".total-days")[0],
    numberOfNights: $(".total-overnights")[0],
    tourOverview: {
        container: document.querySelector('.tour-overview'),
        tableBody: $('.overview-table-body')[0],
        tableRows: $('.overview-day-row'),
        lockButton: document.getElementById("lock-overview"),
        createItinerary: document.getElementById("create-itinerary"),
        update: function(){
            return this.tableRows = $('.overview-day-row');
        }
    },
    tourItinerary:{
        ...
        updateNav: function(){
            return this.navItems = $("#tour-itinerary-tab-nav > .tour-itinerary-tab-item");
        },
        updateTab: function(){
            return this.tabs = $("#tour-itinerary-tabs-content > .tour-itinerary-tab");
        }
    }
}
...
document.querySelector('#create-itinerary').addEventListener("click", function(){
    ...
    if (overviewRows.length === tourData.numberOfDays){
        ...
        initializeItinerary();
        $(guiElements.tourItinerary.container).fadeIn("slow");
    }
});
...
function setVisualDates(){
    if (tourData.startDate != "" && tourData.endDate != ""){
        ...
        $(guiElements.startDate).text(visualStartingDate);
        $(guiElements.endDate).text(visualEndingDate);
    }
}
...
function calculateDaysOvernights(){
    if (tourData.startDate != "" && tourData.endDate != ""){
        ...
        $(guiElements.numberOfDays).text(numberOfDays);
        $(guiElements.numberOfNights).text(numberOfNights);
    }
}
...
function initializeOverview(overviewDayRows, overviewTableBody){
    ...
    for(days = 1; days < tourData.numberOfDays; days++) {
        $(initalRow).clone().appendTo(guiElements.tourOverview.tableBody);
    }
    guiElements.tourOverview.update();
    setDaysInOverview();
    $(guiElements.tourOverview.container).fadeIn("slow");
}
...
function initializeItinerary(){
    ...
    for (days = 0; days < tourData.numberOfDays; days++){
        ...
        $('#tour-itinerary-tabs').tabs("refresh");
        $('#tour-itinerary-tabs').tabs("option", "active", 0);
    }
}
...
<div id="tour-itinerary-tabs-${actualDay}" class="tour-itinerary-tab">
    <div class="overview">
      <h6>Overview</h6>
      <p><b>Tour Day:</b> ${actualDay} <br><b>Tour Date:</b> ${flatpickr.formatDate(tourData.itinerary[i].date, "l - j F, Y")} <br><b>Overnight City:</b> ${tourData.itinerary[i].overnightCity} <br><b>Overnight Hotel:</b> ${tourData.itinerary[i].overnightHotel}</p>
    </div>
